...
//        assert(self.navigationController != nil, "SVWebViewController needs to be contained in a UINavigationController.
        //If you are presenting SVWebViewController modally, use SVModalWebViewController instead.")
        assert(self.navigationController != nil, "SVWebViewController needs to be contained in a")
        updateToolbarItems()
        navBarTitle = UILabel()
    ...
    func updateToolbarItems() {
        ...
        if (UIDevice.current.userInterfaceIdiom == UIUserInterfaceIdiom.pad) {
            ...
            let items: NSArray = sharingEnabled ? [fixedSpace, refreshStopBarButtonItem, fixedSpace, backBarButtonItem,
                                                   fixedSpace, forwardBarButtonItem, fixedSpace, actionBarButtonItem] : [fixedSpace, refreshStopBarButtonItem,
                                                                                                                         fixedSpace, backBarButtonItem, fixedSpace, forwardBarButtonItem]
            ...
            let items: NSArray = sharingEnabled ? [fixedSpace, backBarButtonItem,
                                                   flexibleSpace, forwardBarButtonItem,
                                                   flexibleSpace,
                                                   refreshStopBarButtonItem,
                                                   flexibleSpace, actionBarButtonItem,
                                                   fixedSpace] : [fixedSpace, backBarButtonItem,
                                                                  flexibleSpace, forwardBarButtonItem,
                                                                  flexibleSpace, refreshStopBarButtonItem, fixedSpace]
...
extension SwiftWebVC: WKNavigationDelegate {
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        case "tel":
            openCustomApp(urlScheme: "telprompt://", additionalInfo: urlElements[1])
            decisionHandler(.cancel)
            
        case "sms":
            openCustomApp(urlScheme: "sms://", additionalInfo: urlElements[1])
            decisionHandler(.cancel)
            
        case "mailto":
            openCustomApp(urlScheme: "mailto://", additionalInfo: urlElements[1])
            decisionHandler(.cancel)
